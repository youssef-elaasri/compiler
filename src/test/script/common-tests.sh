...
(
    cd "$test_dir"
    find . -name '*.deca' | egrep -v '^[0-9ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz_/.+-]*$' 2> /dev/null &&
    die "caracteres illicites dans les noms des tests ou des repertoires"
    ...
    (
        ...
        grep -qi "source/syntax-ko.deca:1:" sortie.txt || die "decac -p n'a pas affiché de message d'erreur correctement"
        
        decac -p source/syntax-ok-verif-ko.deca > sortie.txt 2>&1 || die "decac -p a échoué sur un programme invalide mais syntaxiquement correct"
